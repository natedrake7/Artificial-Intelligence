...
def sentence1() -> Expr:
    ...
    B = Expr('B')
    C = Expr('C')
    Prop1 = disjoin(A,B) #A or B
    Prop2 = ~A % (~B | C) #not A if and only if (not B or C)
    Prop3 = disjoin(~A,~B,C) #not A or not B or C
    return conjoin(Prop1,Prop2,Prop3) #return their conjunction
...
def sentence3() -> Expr:
    ...
    Prop2 = ~(A & C)
    Prop3 = C
    return conjoin(Prop1,Prop2,Prop3)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def findModelCheck() -> Dict[Any, bool]:
    ...
    return {dummyClass('a') : True} #return a dictionary of 1 item
    "*** BEGIN YOUR CODE HERE ***"
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    prop = ~(premise >> conclusion) #get the negative of if premise then conclusion
    if findModel(prop) == False : #if the negative is false
        return True #the entailment of the non negative is true
    return False
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    not_inverse_statement = ~inverse_statement #get the inverse of the statement
    if pl_true(not_inverse_statement,assignments) : #if statement is true for the assignments
        return True 
    return False
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    conjunctions = []
    for items in itertools.combinations(literals,2): #get all the combinations of 2 items
        x,y = items #split the tuple
        conjunctions.append(disjoin(~x,~y)) #append the at most one 
    return conjoin(conjunctions) #get a conjunction of all the disjunctions
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    for items in itertools.combinations(literals,2): #get all the combinations of 2 items
        x,y = items #split the tuple
        conjunctions.append(disjoin(~x,~y)) #append the at most one
    conjunctions.append(disjoin(literals)) #append that one of the literals must be true
    #since at least one and at most one must be true,exactly one must be true
    return conjoin(conjunctions) #get a conjunction of all the disjunctions
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    # Add implication for each wall cell (x,y) in all_coords
    for (x, y) in all_coords:
            conjunctions.append(PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str,x,y,time=t))
    pacphysics_sentences.append(conjoin(conjunctions))
    
    conjunctions = []
    # Pacman is at exactly one of the non-outer-wall coordinates
    for (x,y) in non_outer_wall_coords:
        conjunctions.append(PropSymbolExpr(pacman_str,x,y,time=t))
    pacphysics_sentences.append(exactlyOne(conjunctions))

    conjunctions = []       
    # Pacman takes exactly one of the four actions in DIRECTIONS at timestep t
    
    for action in DIRECTIONS:
        conjunctions.append(PropSymbolExpr(action,time=t))
    pacphysics_sentences.append(exactlyOne(conjunctions))
    
    # Add sensor and transition axioms
    if sensorModel :
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    #add pacman coordinates at time 0
    KB.append(PropSymbolExpr(pacman_str,x0,y0,time=0))
    #add pacman action at time 0
    KB.append(PropSymbolExpr(action0,time=0))
    #add pacman action at time 1
    KB.append(PropSymbolExpr(action1,time=1))
    #The goal is to reach location (x1,y1) at time 1
    goal_sent = PropSymbolExpr(pacman_str,x1,y1,time=1)
    #model 1 is that pacman reaches the goal
    model1 = findModel(conjoin(KB + [goal_sent]))
    #model 2 is that pacman doesnt reach the goal
    model2 = findModel(conjoin(KB + [~goal_sent]))
...
def positionLogicPlan(problem) -> List:
    ...
    dirs = []
    for t in range(50):
        #pacman can only be at exactly one location and it cant be a wall
        for (x,y) in non_wall_coords:
            coords.append(PropSymbolExpr(pacman_str,x,y,time=t))
        KB.append(exactlyOne(coords))
        coords.clear()
        #pacman can take exactly one action per timestep
        for action in actions:
            dirs.append(PropSymbolExpr(action,time=t))
        KB.append(exactlyOne(dirs))
        dirs.clear()
        #add tranisition model sentences
        for (x,y) in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x,y,t+1,walls_grid))
        #set the goal state
        goal = PropSymbolExpr(pacman_str,xg,yg,time=t)
        #create a model of all the knowledge base + the goal state
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        #all foods are false so goal is true
        goal = ~conjoin(foods)
        model = findModel(conjoin(KB + [goal]))
        if model:
            return (extractActionSequence(model,actions))
...
def localization(problem, agent) -> Generator:
    ...
    KB = []
    #if there is a wall at (x,y) add it to the knowledge base
    #else add its negative
    for (x,y) in all_coords:
    ...
    for t in range(agent.num_timesteps):
        ...
        #add pacman's action at the current time
        KB.append(PropSymbolExpr(agent.actions[t],time=t))
        #add the four bit percent ruels
        KB.append(fourBitPerceptRules(t,agent.getPercepts()))
        possible_locations = []
        for (x,y) in non_outer_wall_coords:
            #if there is a model ,then we have a possible location
            if findModel(conjoin(KB + [PropSymbolExpr(pacman_str,x,y,time=t)])):
                possible_locations.append((x,y))
            #if our KB entails the pacman pos,add the pos to KB
...
def mapping(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #add pacman pos to the KB
    KB.append(PropSymbolExpr(pacman_str,pac_x_0,pac_y_0,time=0))
    #if there is a wall at pacman location
    #add it to KB
    ...
    for t in range(agent.num_timesteps):
        ...
        #add action to KB
        KB.append(PropSymbolExpr(agent.actions[t],time=t))
        #add 4bitpercent rule
        KB.append(fourBitPerceptRules(t,agent.getPercepts()))
        for (x,y) in non_outer_wall_coords:
            #if there is a model then it is ambigious if we have a wall or not
            model = findModel(conjoin(KB + [PropSymbolExpr(wall_str,x,y)]))
            if model:
                known_map[x][y] = -1
            else:#if there is no model there is not a wall at (x,y)
                known_map[x][y] = 0
            #if KB entails the wall
            if entails(conjoin(KB),PropSymbolExpr(wall_str,x,y)):
                KB.append(PropSymbolExpr(wall_str,x,y)) #add wall to KB
                known_map[x][y] = 1 #there is wall
        #get to next state
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
        yield known_map
...
def slam(problem, agent) -> Generator:
    ...
    for x, y in all_coords:
        if ((x == 0 or x == problem.getWidth() + 1)
            ...
            outer_wall_sent.append(PropSymbolExpr(wall_str, x, y))
    KB.append(conjoin(outer_wall_sent))
    #add pacman starting pos to KB
    KB.append(PropSymbolExpr(pacman_str,pac_x_0,pac_y_0,time=0))
    #if there is a wall at pacman loc add it to KB
    if known_map[pac_x_0][pac_y_0] == 1:
        KB.append(PropSymbolExpr(wall_str,pac_x_0,pac_y_0,time=0))
    "*** BEGIN YOUR CODE HERE ***"
    ...
    for t in range(agent.num_timesteps):
        ...
        #add pacman's action KB
        KB.append(PropSymbolExpr(agent.actions[t],time=t))
        #add the the adjascent perception rules
        KB.append(numAdjWallsPerceptRules(t,agent.getPercepts()))
        #this is the same as q7
        for (x,y) in non_outer_wall_coords:
            model = findModel(conjoin(KB + [PropSymbolExpr(wall_str,x,y)]))
            if model:
                known_map[x][y] = -1
